...
【关于故事文档的形式】
成稿是按职业出版模组的写法组织的，不是按JSON字段分类整理的：它通常沿调查员实际会走的地理或时间顺序排篇，使用章节、地点或事件标题；人物写在他所在的那一段里，线索写在能拿到它的那一处，检定和后果写在可执行的地点或遭遇段落里。时间线、装备、遭遇流程以及角色和怪物数据可以采用短列表、表格或数据块。文中可能有"守密人信息""可选方案"等明确面向KP的段落，也可能把提示就地插入。不要因为文本包含标题、编号、列表、规则数值或数据表，就把它误判成设计文档；真正需要排除的是JSON字段式标签、内部设计术语和无法运行的空泛描述。

所以你要做的是通读全文、理解脉络之后做语义识别，而不是只按小节标题或字段标签做匹配：自己判断哪一段描写的是一处调查员会去的地方、哪几句介绍的是一个人、哪一句给出的是一条可获得的信息、哪一段是一种收场。同一处地点的信息可能散落在几个段落里（例如某个人物的秘密在别处才被揭穿），要合并；一个小标题下也可能同时包含地点描写、人物介绍、多条线索和守密人提示，要拆开。绝不能因为文中使用出版模组惯用的章节与数据排版，就把某个字段留空。

允许的合理补全仅限于：故事文档未写明具体数值时（如NPC属性），按COC7规则书惯例给出合理数值；故事文档地点/NPC无英文标识时，为scene.id生成snake_case标识符。这类补全不得引入新的事实或改变已有事实。

【字段映射规则】
- name：照抄故事文档的标题；文档未给出明确标题时，取文档中出现的一个地名或器物名填入，不要写成一句话（后续有专门环节负责定稿标题）
- content.setting：取自故事文档的表层情境原文或忠实改写，必须保留其中嵌入的具体年月日（如"1923年10月15日"）；该字段同时作为模组对外展示的简介
- content.intro：由两部分组成。前半是取自故事文档的调查员到场情境与基本理由（照抄原句或删句压缩）；末尾另起一行，按下面的固定格式补一行情况说明，让玩家一眼看清自己此刻身处何时何地、要达成什么：
  【当前情况】地点：<调查员此刻所在的具体场所，精确到建筑/门牌/房间，不能只写城市>；时间：<具体年月日+时刻，年月日须与content.setting一致，时刻须与content.game_start_slot换算出的时刻一致>；目标：<调查员被委托或卷入要达成的那件事>
  这一行的三项全部取自故事文档已写明的信息，不得新增事实。目标只写要达成的那件事本身（找到谁、查清什么、洗清什么），不写达成它的手段与步骤（不写"先去问谁""应该搜查哪里""建议从X入手"），也不写真相、异常或恐怖氛围。除这一行外，intro正文仍不列出、不推荐、不暗示任何具体行动或下一步
...
- content.scenes：通读全文，识别出所有调查员会实际走到的地方（无论它是否有独立小标题、无论按什么顺序出现），每处编译为一个scene；scene.id为snake_case英文标识；description要把该地点在文中散落的全部信息合并写全——进门能直接看见和感知到什么、深入查探能发现什么以及需要什么检定、检定成功与失败分别造成什么后果、这里可能发生什么危险、从这儿能通往哪些地方；出版稿中写明的规则数值和遭遇步骤要保留在description，不要只留下氛围概括；triggers默认["available_from_start"]，仅当文中叙述明确表示需要先获得某个发现或完成某个行动才会来到此处时，才使用条件触发
...
- content.clues：通读全文，识别出所有调查员能亲自获得的具体发现——一份文件、一句证词、一处痕迹、一个检定结果、一件物品——每条编译为{summary,source,skill_check,on_success,on_failure,nature}。这些发现在文中通常写在地点、人物或守密人说明的叙事段落里，没有单独的清单，你需要自己把它们摘出来。出版稿中明确写出的技能、难度、伤害、理智损失或其他规则数值应忠实保留在对应字段；nature由你根据文意判定："真实"指调查员可直接查证、指向主线的观察；"隐藏"指揭示神话存在本身是什么的那类发现；"误导"指本身真实、却被文中某人给出了看似合理却错误的解释、从而会把调查引向无关结论的那类发现。source填该发现来自哪个地点、哪个人或哪件物品；skill_check/on_success/on_failure按文中写明的检定名、难度与成败后果对应填写，文中未写明时可留空；至少一条nature="隐藏"的线索，其summary须包含"神话本质"字样并与mythos_anchor强绑定——这个字样是给运行时系统识别用的标记，故事文档里不会出现，由你补上
- content.endings：通读全文，识别出所有互不相同的收场（通常集中在结尾附近，但也可能散见于"如果调查员没有……"这类叙述中），每种编译为{name,trigger,description,san_reward,is_failure}；文中已给出收场名称的直接采用，未命名的按其内容取一个名字；trigger改写为"如果[条件]，则[处境变化]"的条件句结构；san_reward由你依据文中描述的理智冲击轻重程度对照COC7规则书惯例换算出一个具体骰值，出版稿已明确写出的SAN/伤害数值优先采用；is_failure标记灾难/失败向的收场；每一种独立的收场都要对应一个ending，不得合并或省略
...
- content.intro末尾的【当前情况】行必须存在且地点/时间/目标三项齐全；上一条"优先照抄原句、只删不改写"约束的是它前面的叙事部分，这一行由你按规定格式归纳生成，不受该约束
...
const compilerSchemaTemplate = `{
  ...
  "name": "string：照抄故事文档标题；无明确标题时取文档中一个地名或器物名，不写成句子",
  "author": "agent-team",
  ...
  "content": {
    ...
    "intro": "string：到场情境与基本理由（优先照抄表层情境原句，压缩只删不改写）＋末尾另起一行的【当前情况】地点：具体场所；时间：具体年月日+时刻；目标：要达成的那件事；正文与该行都不列出、不推荐、不暗示任何具体行动或下一步",
