...
func checkRuleTool() scripterTool {
	return scripterTool{
		def: llm.ToolDefinition{
			...
			Description: `查询规则专家(必须用于所有规则判定,不可自行编造)。规则专家只能检索COC 7th核心规则书/法术书/怪物图鉴,没有<scenario>剧本文本的访问权限,问题范围严格限于:
✓ COC 7th 核心规则书内容(技能判定、对抗、伤害、疯狂、追逐、战斗等机制)
✗ 不能回答当前模组/剧本中的设定、线索、NPC 资料、场景描述等剧情内容——你自己在<scenario>标签里已有完整原文,直接查阅即可,不要为此调用check_rule;规则专家读不到<scenario>,问了只会得到"以剧本内容为准"的空回复,白白浪费一轮调用
