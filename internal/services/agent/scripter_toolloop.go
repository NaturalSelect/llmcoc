...
func getWritingExampleTool() scripterTool {
	return scripterTool{
		...
		def: llm.ToolDefinition{
			...
			Description: `获取一份职业出版模组的完整成稿作为写作参考，学习它如何用章节和地点标题组织阅读顺序，如何把环境、人物、发现、检定、遭遇与去向写在同一地点段落中，以及如何安排守密人信息、时间线、可选方案和角色/怪物数据。
不要照抄其中具体的人名、地名、机构名、情节或神话设定——你要写的是一份完全不同的剧本。参考成稿中的规则数值、骰值和数据表是出版模组的正常组成部分，可以学习它们如何服务于运行，但必须按本篇已核验的COC7事实重新编写，不能照抄；你的正文也不要写JSON字段、内部设计术语或机械化要素表。必须在正式动笔前调用一次，无需参数。`,
			Parameters: jsonSchemaObject(`{
				"type": "object",
				"properties": {}
			}`),
