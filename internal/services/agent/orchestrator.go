...
func buildNPCDetail(npcName string, tempNPCs []models.SessionNPC, scenarioNPCs []models.NPCData) string {
	...
	for _, npc := range tempNPCs {
		...
		sb.WriteString(fmt.Sprintf(`<npc n=%q race=%q src="tmp" st=%q>`, npc.Name, npc.Race, status))
...
func buildPlayerBrief(players []models.SessionPlayer) string {
	...
	if hasNotHuman {
		s += "\n\n <attention>非人类角色, 仍然适用于疯狂规则(损失过多进入疯狂)和SAN损失规则(但不受克苏鲁神话对最大理智的限制), 但其SAN实际上代表人性, 施法和使用种族能力会有额外50%人性(理智)损失(至少1点), 除此之外外貌按人类标准处理, 物理接触也不会有特别的效果(食尸鬼等规则书明确标注的除外)。非人类调查员的其他特性与常人无异, 因为他们需要生活在人类社会中。</attention>\n"
	}
	return s
}
