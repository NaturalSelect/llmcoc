...
type ToolCall struct {
	...
	NSFW          bool                   `json:"nsfw"`           // NOTE: generate_image: 该配图是否含裸露/性暗示等NSFW内容(可选,默认false);管理员未放行NSFW配图时,标记为true的调用会被拒绝。write: 该段描述是否含色情/性描写内容(可选,默认false),仅在房间NSFW开关开启时生效。act_npc: 该次互动是否含色情/性描写内容(可选,默认false),仅在房间NSFW开关开启时生效
