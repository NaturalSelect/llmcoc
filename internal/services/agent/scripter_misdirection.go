...
<task>完成两件事：
①将IronyCore的δ算子结构翻译为COC7可运行的神话机制（锁定mythos_anchor）；
②设计系统性的误导结构（false_lead、misdirector_npc、true_trace、reveal_trigger、retrospective_key），确保false_lead在delta_true框架下后验兼容。
</task>
<response_format>json_object</response_format>
<output>只输出合法JSON对象，不要Markdown、标题、解释或代码围栏。</output>
<schema>{
  "false_lead": "一条有说服力的假线索，将调查员推向false_delta；必须在deep_truth框架下也有合理解释",
  ...
  "mythos_anchor": "已核验或保守标注的COC7神话锚点",
...
<rules>
- false_lead必须满足后验必然性：在deep_truth框架下，这条线索必须有合理解释（P(false_lead|delta_true)>0）。
- misdirector_npc应有内在动机支持false_delta解读，不依赖"他是坏人"这种纯功能性设定。
- true_trace必须是歧义证据：表面支持false_delta，但只需一个额外上下文就能转向deep_truth。
- mythos_anchor一旦写定后续阶段不得更换；不确定时在rules_notes显式写"不确定/保守处理"。
...
const misdirectionQASystemPrompt = `<role>COC7沙盒误导结构QA</role>
<task>审核MisdirectionFabric是否满足：false_lead后验兼容、misdirector_npc动机合理、派系自主行动、干预枢纽具体。不审核规则书内容（已在Stage2锚定）。</task>
...
<audit_rules>
审核四点，任意一点不满足则pass=false：
1. false_lead后验兼容：false_lead在deep_truth框架下必须有合理解释（不能有"在δ_true下完全说不通"的假线索）。如果false_lead只在false_delta框架下有意义，在deep_truth下完全矛盾，pass=false。
2. misdirector_npc动机合理：misdirector_npc必须有内在动机支持false_delta解读（被动的存在或行为即可），而不是被写成"专门误导调查员的功能性NPC"。
