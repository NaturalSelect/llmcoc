...
func buildKPMessages(gctx GameContext, systemPrompt string, history []llm.ChatMessage, tempNPCs []models.SessionNPC) []llm.ChatMessage {
	...
	userSB.WriteString("The hit tool call is used to write workflow hint(e.g. **ALREADY**  change A' HP and B' MP and C's SAN, don't reduce in next message) to avoid duplicate stat update\n")
