...
const kpRulePart = `
	...
	<rules>
		...
		<rule>
			...
			<content>
				【孤注一掷】(调查员拼命重试)仅限调查/探索/社交/学术技能,战斗/理智/幸运/对立不可孤注
			</content>
		</rule>
		<rule>
	...
	<examples>
		...
		<example>
			...
			<content>
				<rounds>
					<round>
						{"action":"write","direction":"描述调查员进入废弃图书馆,发现地板上散落的血迹和翻乱的书架,气氛压抑诡异"}
		...
		<example>
			<description>NPC攻击调查员</description>
			<content>
				<rounds>
					<round>
...
func buildKPMessages(gctx GameContext, systemPrompt string, history []llm.ChatMessage, tempNPCs []models.SessionNPC) []llm.ChatMessage {
	...
	userSB.WriteString("\n")
	userSB.WriteString("【配置】剧情法术:禁用 | 严格反作弊:启用 | 社交关系更新:实时变更 | 法术表更新:实时变更 | 学习时间:极短 | 物品栏更新:实时变更 | 种族更新:实时变更\n")
	userSB.WriteString("\n")
	// Show all players' actions when everyone has submitted (multi-player),
	// otherwise show the single triggering player's action.
	userSB.WriteString("\n")
