...
- 时间线节点用中性事实记录句书写（谁在何时做了什么、什么状态发生了变化）；不写成对话转录、不用引号引用人物原话、不写成文学化叙事句——对话与场面留给地点/NPC/线索部分
...
【步骤④之二：游玩大纲】
故事文档必须包含一个以"游玩大纲"为小节标题的独立段落，把已设计的场景、NPC、线索串成一条可执行的游玩流程，供KP照此推进——不是重复地点/NPC/线索部分已写过的详细描述，只写流程与衔接关系：
- 开场入口：调查员如何从表层情境进入第一个场景
- 逐场景条目：按调查推进的大致顺序，每个场景写明①场景名 ②入口条件（默认开局即可进入，或需要何种线索/行动解锁）③可接触的NPC ④可获得的线索 ⑤出口去向与分支（做出什么行动或获得什么线索会通向哪个场景；有分支须写"若…则…"）
- 结局收束：哪些条件分别导向哪个已命名结局
- 大纲中出现的场景、NPC、线索、结局必须全部来自本文档已设计的内容，不得新增
...
✓ 表层情境（对应最终setting/intro）：中性日常语气的冷开场——只交代时代、地点、调查员为何到场；读者/玩家从中看不出剧情、案件、真相、神话或恐怖走向，不带任何惊悚、诡异、压抑或不祥的氛围词；须嵌入与时代、地点及剧情氛围一致的具体年月日（如"1923年10月15日"）；不列出、不推荐、不暗示任何具体行动或下一步，行动入口留给玩家自行探索
...
✓ 游玩大纲：以"游玩大纲"为小节标题，按开场→逐场景（入口条件/可接触NPC/可得线索/出口去向与分支）→结局收束的顺序，串起全部场景、NPC与线索
✓ 时间线：过去线痕迹 + 现在线推进 + 调查员可执行的干预点；每个节点是中性事实句而非对话转录
...
✓ 若<length_spec>要求提供时间线节点/守秘人附录/导入身份表/量化机制，是否已按要求提供，且内容具体、未为凑数硬写？
...
<scope>
- 编号/模板腔禁令只适用于面向玩家的表层情境段落（对应最终setting/intro）
- KP内部真相、地点、NPC、线索等结构化内容段落可以保留"可见/可发现/杠杆/公开身份/议程/秘密"等要素标签，不要因此报问题；但要素内容若空泛套话（如"异常的气味""神秘的声音"）应报问题
- 与空泛同级的问题是堆砌：一句话并列三个以上互不相关的具体细节（专名、属性、动作），细节间无因果/感知/情绪连接，读起来像道具清单或人物卡直译——应报问题，判定标准见standards中"一段一焦点""细节要挂钩"
