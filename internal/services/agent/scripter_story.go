...
不要输出JSON、字段名或内部设计标签。成稿应使用清晰的Markdown标题、章节、短列表和必要的数据块，像出版模组一样方便守密人翻阅与运行；不要为了方便后续编译器而把正文改写成字段表。后续有专门的编译器阅读你的成稿，它会从出版体例的叙述、标题、列表和数据块中识别地点、人物、线索、结局等内容。
...
- 到场理由的强度看调查员实际领受的具体任务，不是背景里一笔带过的某人安危/失踪/死亡：若背景只提"某人失联"，调查员却只是受托去做设备交接、退租、遗物清点这类与查明下落无关的事务性善后，本质仍是"帮忙送/取东西"的变体，同样禁止——委托本身就必须是去弄清这个人出了什么事，安危/失踪要成为调查员要解决的任务，不能只是一句背景交代
- 入场原因的具体设定应参考典型洛氏小说的经典开场模式，例如：受友人来信或同僚委托追查一桩失踪、追查一桩反常事件的报道等；从中选一个贴合brief与恐怖内核的具体钩子加以改写，不要脱离这类传统另起炉灶
...
- 检定和它的后果写进叙事句里："成功的心理学检定可以发现彼得显然不太喜欢死者""通过困难难度的侦查检定可以发现南侧围墙上有一个三米宽的缺口""失败的调查员会从楼梯上一脚踏空摔下去，受到1D6点伤害；如果过了一个跳跃检定，则能安全着陆"。检定名、难度、成功与失败的结果都在同一句话里自然出现；出版模组通行的做法是同时写清具体状态感受（伤在哪、还能不能动、看见听见了什么、心智被冲击成什么样）和对应的规则数值（伤害骰值、理智损失骰值），两者不互相排斥；伤害、理智冲击的轻重程度拿不准时用ask_lawyer核实该检定/伤害/法术效果在规则书里的实际量级，核实到的数值直接写进正文，不要凭印象编造
...
【禁止的写法】
- 不写JSON字段式标签：任何形如"可见信息：""可发现信息：""杠杆：""风险：""出口：""感官细节：""公开身份：""议程：""秘密：""动机：""关系网：""初始态度：""性质：真实／隐藏／误导""触发条件：""表面假象：""错误解释："的行首标签一律不许出现。这些内容应当融入完整句子、地点段落或守密人提示中
- 不写设计术语：核心线索、辅助线索、误导线索、红鲱鱼、承载者、线索矩阵、施动者、宇宙公理、洋葱式结构、恐怖渐进、四要素——这些是构思时的工作语言，不是读者该看到的字；若守密人必须理解某个运行机制，用玩家能理解的具体行动和后果解释
- 不把出版排版误当成模板腔：章节标题、地点标题、"守密人信息"、"时间线"、"结局"、角色与怪物数据、装备清单和完整的遭遇步骤都可以出现；编号章节和短列表也可以使用，但不要把每一处都机械排成相同字段
...
✓ 开头有一段调查员的导入，用平静的日常语气写，并且这三件事都写得明明白白、不用读者自己去猜：调查员此刻人在哪个具体场所（精确到建筑、门牌或房间，不能只写城市）、此刻是哪年哪月哪日的什么时刻（如"1922年2月17日上午十点"）、以及他们被委托或被卷入要达成的是什么事（找到谁、查清什么、洗清什么）。要达成的那件事只写它本身，不写该怎么去做。读这一段的人看不出这里会出事，读不到任何惊悚、诡异、压抑或不祥的字眼，也读不到"接下来该做什么"的建议——行动入口留给玩家自己撞上
✓ 调查员的到场与留守理由足够强：不是"帮忙送/取东西"式的可有可无委托，也不是把某人安危/失踪当背景一笔带过、实际领受的任务却只是设备交接/退租/遗物清点这类事务性善后；即使中途察觉不对劲也有牵绊让他们没法转身就走
...
✓ 时间线写成中性的事实记录（谁在什么时候做了什么、什么状态变了），带具体日期；不写成对话，不用引号引用人物原话。时间线可以使用出版模组常见的项目符号或表格式短行，但每项都必须是可供守密人使用的事实
...
✓ 正文中的检定、战斗、法术、理智、伤害和角色数据具有可执行性；规则数值可以出现在相应的模组段落和数据块中，不把出版模组需要的规则信息模糊成纯文学描写
...
// 成稿是否具备出版模组应有的章节/地点划分（不校验标题语义，只做结构性下限检查）。
func markdownHeadingCount(document string) int {
	count := 0
...
// storyQAReviewSystemPrompt 与故事 architect 共用 humanWritingRules，保证审查标准一致，并专门检查出版模组的可运行体例。
func storyQAReviewSystemPrompt() string {
	return `<role>出版模组体例与人写化审查员</role>
<task>审查COC剧本故事文档是否像一份可供守密人直接使用的出版模组，并输出必须整改的问题清单。重点审文字质感、信息组织和运行可读性；不重新设计剧情，不替代规则逻辑审查。</task>
<standards>
` + humanWritingRules + `
</standards>
<scope>
- 反模板腔的要求只针对JSON式字段表，不针对出版模组的正常排版：地点、人物、线索与结局可以通过标题和正文自然组织，也可以在时间线、装备、遭遇流程、角色与怪物数据处使用短列表、表格或数据块；审查重点是信息是否具体、可读、可执行，而不是是否完全没有编号
- 不要把规则数值误判成AI腔：出版模组会在检定、战斗、法术、理智、伤害、遭遇和角色数据中明确写出技能难度、骰值、属性、技能、生命值、护甲与特殊能力；这些内容只要与上下文和COC7规则一致，就不应报告为问题。真正要报告的是规则数值缺失、结果不可执行、数值与叙述互相矛盾，或把规则表机械复制进不相关的环境描写
- 检查每个主要地点是否像出版稿一样能直接运行：抵达时的环境、可调查内容、检定与成败后果、可能遭遇、离开与前往其他地点的方式至少应交代主要部分；不要求每处地点机械使用完全相同的小标题
- 开头的导入段必须保持日常、平静、无恐怖氛围、不剧透真相，并且把调查员所在的具体场所、具体年月日与时刻、他们要达成的那件事都写明白；可以使用章节标题或出版稿的导语排版，但不要写成JSON字段表
- 明确写给守密人的信息可以集中在"守密人信息""可选方案""时间线"等段落，也可以就地插入"守密人应当……"的提示；检查它是否说明了运行时机，而不是要求所有提示都藏在散文中
- 财务审计类桥段不止看字面是否出现"账目""查账"：换成"核对账本""稽核收支""对流水""审计""对账"等同义说法，只要调查手法的实质仍是核对财务记录，同样要报告为问题
- 涉及未成年人性内容、人兽等严重不当内容同样不止看字面用词：换用隐晦说法或委婉描述来实现同样的内容实质，同样要报告为问题
- 到场理由要看调查员实际领受的任务和正文实际展开的开场行动，不能只看是否用了"查明下落""确认安危"这类字面措辞：如果名义委托是查明某人安危/失踪/死亡，但正文实际展开的开场行动仍是处理房产、设备交接、退租、遗物清点这类事务性杂务，且这些杂务占据了开场的主要篇幅，仍要报告为到场理由不够强，本质仍是变相的跑腿型委托
</scope>`
}
