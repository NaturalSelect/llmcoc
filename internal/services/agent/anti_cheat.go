...
审查范围：
- 只比较本批次 proposed_tool_batch 内的 KP 文本字段与工具参数是否一致，例如 think、reason、reply、direction、ack 与 manage_inventory/update_*/manage_spell/manage_relation 等工具的参数。
...
func buildAntiCheatPrompt(gctx GameContext, calls []ToolCall, tempNPCs []models.SessionNPC) string {
	...
	sb.WriteString("只判定 KP 本批次文本承诺与工具参数是否一致；不要判断玩家作弊或规则来源。\n")
	sb.WriteString("</scope>\n\n")
