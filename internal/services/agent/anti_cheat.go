...
- 只在存在清晰、可指出的自相矛盾时返回 must_fix：例如合约/think/reason 明确承诺“不增强/仅叙事换皮/保持原属性/不改变数值/不授予物品/不造成伤害/不更新关系”，但后续副作用工具参数实际增加伤害骰、护甲、奖励骰、数量、属性、法术、关系、HP/SAN/MP变化等机械收益或损失。
- 如果 KP 的 reason 声称执行 A，但工具参数执行 B，也返回 must_fix。
- 如果 KP 的 think 显示向玩家妥协而放弃了原本合理的拒绝（例如玩家要求“我希望这个物品有X能力”，KP 原本可以拒绝但改成了“虽然规则里没有，但我觉得合理，所以就给你这个能力吧”），也返回 must_fix。
...
func checkAntiCheat(ctx context.Context, h agentHandle, gctx GameContext, calls []ToolCall, tempNPCs []models.SessionNPC) (AntiCheatVerdict, bool, string) {
	...
	if !hasNonEmptyThink(calls) {
		verdict := AntiCheatVerdict{
			...
			Message: "每一个消息是一个批次，本批次包含副作用工具，含有副作用的batch必须包含一个 think 调用并在其中写明 ANTI_CHEAT_CONTRACT，重试本批次将think和要进行的副作用必须被放在同一批次(单独的think没有任何作用且将被禁止),以通过系统检查。重发相同的消息会被直接拒绝，修改后重试。\n唯一有效的形式: [think, other actions]\n 认真看这条错误消息，想清楚再回答，再下一个回答的think中包含你对这条消息的理解, 禁止单回一个think\n下次搞不定就关机吧,垃圾模型",
		}
