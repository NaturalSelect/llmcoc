...
- 绝对禁止把 respond 和 contract/ask_lawyer 放在同一个JSON数组中。错误示例：[contract, ask_lawyer, respond]。
...
func runRewardAgent(ctx context.Context, room *scripterRoom, concept, mythosAnchor string) (*models.ScenarioReward, error) {
	...
	for round := 1; round <= maxRounds; round++ {
		...
		raw, err := provider.provider.JsonChat(ctx, provider.cacheKey(sessionIDFromContextValue(ctx)), msgs)
		if err != nil {
			return nil, err
		}
		...
		if rewardRespondMixed(calls) {
			msgs = append(msgs, llm.ChatMessage{Role: "user", Content: "SYSTEM REJECT: respond必须单独一轮输出，不能和contract、ask_lawyer或任何其他action混在同一个JSON数组中。若还需查询，本轮只输出ask_lawyer；若已有足够信息，下一轮只输出一个respond。"})
			continue
		}
		...
		for _, call := range calls {
			...
			default:
				msgs = append(msgs, llm.ChatMessage{Role: "user", Content: fmt.Sprintf(
					"SYSTEM REJECT: reward_agent只允许contract/ask_lawyer/respond，不允许%s。", call.Action)})
				invalid = true
			}
		}
		if invalid {
