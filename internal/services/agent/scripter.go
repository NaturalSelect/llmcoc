...
- 再通过 search_rule 工具调用， 检索规则书原文以核实细节和数值，避免凭空编造。搜索结果会原样反馈给你，帮助你做出正确的设计决策。
- 一轮可包含多个 search_rule/read_rulebook_const
- 一旦获取了所有需要的信息，就需要通过 answer 输出完整大纲，结束本阶段
...
【执行规则】
- 每次输出必须是 JSON 数组
- 先通过 search_rule/read_rulebook_const 核实模组中涉及的怪物、法术、技能等是否合规，再输出 answer
- 一轮可包含多个 search_rule/read_rulebook_const，或单个 answer，不混用
- 仅输出 JSON 数组，不加任何说明文字
...
func RunScripterScenarioTeam(ctx context.Context, req ScenarioCreationRequest) (ScenarioCreationOutput, error) {
	...
	for i := 0; i < 3; i++ {
		...
		qaResult, err = runQA(ctx, qaAgent, parser, req, draft, idx)
		if err != nil {
...
func runQA(ctx context.Context, qaAgent agentHandle, parser agentHandle, req ScenarioCreationRequest, draft ScenarioDraft, idx rulebook.Index) (qaGuardResult, error) {
	reqJSON, _ := json.Marshal(req)
	...
	for iter := 0; iter < maxIter; iter++ {
		...
		for _, c := range calls {
			if c.Action == "answer" {
				...
				// result field failed to parse in pipelineToolCall — extract raw answer JSON and repair
				log.Printf("[qa] iter=%d answer c.Result==nil，尝试从原始输出解析", iter+1)
...
func executeSearchCalls(calls []pipelineToolCall, idx rulebook.Index, tag string) string {
	...
	for _, c := range calls {
		...
		case "search_rule":
			...
			sb.WriteString(fmt.Sprintf("【search_rule:%s】\n%s\n\n", c.Keyword, text))
			debugf("SearchRule", "keyword: %v content: %v", c.Keyword, text)
