...
const scenarioExample = `{
  ...
  "content": {
    ...
    "game_start_slot": 16,
    "setting": "1923年,某地。详细的时代/地点背景描述(简短但包含必要信息)……",
    "intro": "开场叙事(介绍调查员的任务, 剧本的开场信息, 简短但包含必要信息),以第二人称描写……",
...
func RunScripterScenarioTeam(ctx context.Context, req ScenarioCreationRequest) (ScenarioCreationOutput, error) {
	...
	outline, err := generateOutline(ctx, architect, req, npcNameBlacklist, recentScenarioNames)
	if err != nil {
	...
	draft, err := buildDraft(ctx, architect, parser, outline, req.TargetLength, npcNameBlacklist, recentScenarioNames)
	if err != nil {
	...
	for i := 0; i < 3; i++ {
		...
		qaResult, err = runQA(ctx, qaAgent, parser, req, draft, npcNameBlacklist, recentScenarioNames)
		if err != nil {
		...
		revised, revErr := reviseDraft(ctx, architect, parser, draft, qaResult.MustFix, outline, npcNameBlacklist, recentScenarioNames)
		if revErr != nil {
...
func buildDraft(ctx context.Context, architect, fixer agentHandle, outline string, targetLength string, npcNameBlacklist []string, recentScenarioNames []string) (ScenarioDraft, error) {
	userMsg := fmt.Sprintf(draftPrompt, outline, scenarioExample, lengthSpec(targetLength))
	userMsg += "\n\n【近期剧本主题/标题黑名单，禁止 name/description/setting/core mystery 与其近似】\n" + formatScenarioTitleBlacklist(recentScenarioNames)
...
func runQA(ctx context.Context, qaAgent agentHandle, parser agentHandle, req ScenarioCreationRequest, draft ScenarioDraft, npcNameBlacklist []string, recentScenarioNames []string) (qaGuardResult, error) {
	reqJSON, _ := json.Marshal(req)
	draftJSON, _ := json.Marshal(draft)

	userMsg := fmt.Sprintf("审查以下 COC 模组的质量, 是否符合逻辑, 剧情是否胡乱编造。\n\n【原始需求】\n%s\n\n【近期剧本主题/标题黑名单，模组不得同名、近似标题或核心主题高度相似】\n%s\n\n【近期已用 NPC 名字黑名单，npcs[].name 禁止复用】\n%s\n\n【模组草案】\n%s",
		string(reqJSON), formatScenarioTitleBlacklist(recentScenarioNames), formatNPCNameBlacklist(npcNameBlacklist), string(draftJSON))
...
func reviseDraft(ctx context.Context, architect, fixer agentHandle, draft ScenarioDraft, mustFix []string, outline string, npcNameBlacklist []string, recentScenarioNames []string) (ScenarioDraft, error) {
	draftJSON, _ := json.Marshal(draft)
	issues := strings.Join(mustFix, "\n- ")

	userMsg := fmt.Sprintf(revisionPrompt, outline, string(draftJSON), issues, scenarioExample)
	userMsg += "\n\n【近期剧本主题/标题黑名单，修订后的 name/description/setting/core mystery 禁止与其近似】\n" + formatScenarioTitleBlacklist(recentScenarioNames)
