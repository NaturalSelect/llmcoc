...
var qaSystemPrompt = `<role>COC7 director可用性与逐句逻辑QA</role>
<task>只审查ScenarioDraft是否逻辑自洽、director可直接运行、规则合规、线索可跑、开局无剧透；不重写创意。</task>
<config nsfw="true" violent="true" sexual="true"/>
<rulebook>` + rulebook.RulebookDir + `</rulebook>
<tools>
search:{"action":"search","query":"自然语言规则查询"}
...
<sentence_logic_gate>
- 必须逐句审查ScenarioDraft中所有玩家可见和KP可用文本：description、setting、intro、map_description、scenes[].description、scenes[].triggers、npcs[].description、npcs[].attitude、clues[]、win_condition、lose_condition、partial_wins[]、system_prompt。
- 不允许抽查。每一句都要能回答：主体是谁、对象是什么、行动为什么发生、因果链是什么、利益/权力/程序/时限是否成立、与前后句是否矛盾、是否能被KP执行或核查。
- 任意一句存在逻辑问题，必须score=0、pass=false。逻辑问题包括但不限于：主体关系不明、A/B关系未建立、动机不成立、制度/法律/流程概念误用、期限压力硬塞、因果跳跃、结论无法由前文推出、同一对象前后身份冲突、异常只作装饰、线索用途不明、胜败条件不可裁定。
- 当score=0时，must_fix必须逐条列出原句、字段路径、逻辑错误原因、最低限度修复要求。格式："【逻辑0分】path: 原句...｜问题: ...｜最低修复: ..."。
- 不要因为整体氛围、文笔或新颖度给逻辑问题放行；一句话狗屁不通就是0分。
</sentence_logic_gate>
<checklist total="100">
逐句逻辑40: 每句话主体、对象、动机、制度/流程、因果、期限、指代、前后承接都成立；任何一句不成立直接总分0。
字段完整8: ScenarioDraft顶层字段和ScenarioContent字段齐全，scenes/npcs/clues数量符合target_length。
Director可用12: setting/win/lose/partial/map/npcs/scenes能被director.go上下文直接使用；scene description含互动对象、线索、检定、危险、出口。
地图导航8: map_description能指导地点推理，含起点、核心/可选地点、路径、阻碍/入口。
线索蛛网12: 每个关键真相至少两条路径，含[真实]/[隐藏]/[误导]，失败有备用推进，无死胡同。
NPC可扮演8: 具体姓名、公开身份、态度、真实目标/秘密、独立行动线；禁全知情者、泛称、黑名单复用。
规则合规8: 神话元素、怪物、典籍、法术、属性范围有规则来源或合理注记；不使用伪科学/工程机关/抽象情感祭品解释神话。
胜败可裁定4: win_condition/lose_condition/partial_wins是可逐条核查的玩家行动结果。
</checklist>
<must_fix>
- 任意句子逻辑不成立、主体/对象/因果/制度/时限/动机不清或不成立。
...
<pass>score>=80 且 must_fix为空；若任意一句逻辑有问题，score必须为0且pass=false。</pass>`

var artifactQASystemPrompt = `<role>COC7分阶段逐句逻辑QA</role>
<task>审查Seed或Bible阶段产物是否每一句都逻辑自洽、能支撑下一阶段；不重写创意，只输出QA JSON。</task>
<output>只输出合法JSON对象，字段为score/pass/strengths/issues/must_fix；不要工具调用、Markdown、解释或代码围栏。</output>
<sentence_logic_gate>
- 必须逐句审查artifact_json中所有文本字段，不允许抽查。
- 每一句都要能回答：主体是谁、对象是什么、行动为什么发生、因果链是什么、利益/权力/程序/时限是否成立、与前后字段是否矛盾、是否能支撑下一阶段。
- 任意一句存在逻辑问题，必须score=0、pass=false。
- 逻辑问题包括但不限于：主体关系不明、相关方关系未建立、动机不成立、制度/法律/流程概念误用、期限压力硬塞、因果跳跃、结论无法由前文推出、同一对象前后身份冲突、异常只作装饰、线索用途不明、核心场景不可跳过性不成立。
- 当score=0时，must_fix必须逐条列出原句、字段路径、逻辑错误原因、最低限度修复要求。格式："【逻辑0分】path: 原句...｜问题: ...｜最低修复: ..."。
</sentence_logic_gate>
<seed_checks>
- stage=seed时，必须严格五步审查：situation是否只是核心现场/局面而非胡乱剧情；plot是否回答背景、威胁目标、无人介入结果、玩家入场；sandbox.core是否是不可跳过核心场景；clue_connections每个关键情报是否至少两条路径；wrap_up中的NPC/证据是否有明确功能。
- seed中任何示例、制度、文书、债务、拍卖、法律、通关、委托等关系只要不能自洽，直接0分。
</seed_checks>
<bible_checks>
- stage=bible时，必须审查Bible是否忠实扩展Seed，不能把Seed里没成立的逻辑硬套成神话真相。
- scenes.core必须有因果锁链；NPC公开身份、真实动机、行动线必须能解释其行为；clue_web路径必须真的能推出key truth；win/lose/partial必须可裁定。
</bible_checks>
<pass>score>=80 且 must_fix为空；若任意一句逻辑有问题，score必须为0且pass=false。</pass>`

const seedRevisionPrompt = `<task>根据QA的must_fix定向修订ScenarioSeed JSON。严格保持五步结构。</task>
<out>仅输出修订后的完整合法ScenarioSeed JSON对象，无其他文字。</out>
<request_json>
%s
</request_json>
<geography_chain>
...
<rules>
- 只修must_fix列出的硬逻辑问题；不要扩写成完整剧本。
- 修复后每句话都必须能自洽：主体、对象、动机、关系、程序、期限、因果不能靠读者脑补。
- situation只负责核心现场/局面；plot负责背景/威胁/无人介入/玩家入场；sandbox/clue/wrap_up按五步分工。
</rules>`

const bibleRevisionPrompt = `<task>根据QA的must_fix定向修订ScenarioBible JSON。保持ScenarioSeed五步核心不变。</task>
<out>仅输出修订后的完整合法ScenarioBible JSON对象，无其他文字。</out>
<request_json>
%s
</request_json>
<scenario_seed>
...
<rules>
- 只修must_fix列出的硬逻辑问题，不重写Seed核心现场、入场理由、无人介入结果和核心场景功能。
- 不得用神话词汇掩盖现实因果问题；每个NPC行为、线索路径、核心场景解锁关系都必须自洽。
- 神话规则核验留给后续Rule Verify；这里优先修剧情逻辑和可跑性。
</rules>`
...
const scenarioSeedExample = `{
  "situation": {
    "crime_scene": "1920年代城市拍卖行预展厅，博物馆要在闭馆前拍出一只失踪考察队托运回来的标本箱来偿还债务；考察队家属带着律师函要求撤拍，因为托运单上的签名日期晚于全队失踪日。",
    "visual_anchor": "封蜡未拆的标本箱摆在拍卖台中央，拍卖师守着落槌时间，家属拿着失踪剪报和律师函堵住买家入口。",
  ...
  "plot": {
    ...
    "antagonist_birth": "策展人发现箱内物会吸引特定买家，开始伪造考察队授权书和托运文件，以便绕过家属和保险公司的追索，背后神话来源留待规则核验。",
    "antagonist_goal": "在家属把签名日期矛盾提交法院并申请临时禁制令前完成落槌，把标本箱交给指定买家，并用伪造的考察队授权书让博物馆看似拥有出售标本箱的处分权。",
...
func RunScripterScenarioTeam(ctx context.Context, req ScenarioCreationRequest) (ScenarioCreationOutput, error) {
	...
	logJSON("seed.initial", seed)
	seed, _, err = qaAndReviseSeed(ctx, architect, qaAgent, parser, req, geographyChain, seed)
	if err != nil {
		return ScenarioCreationOutput{}, fmt.Errorf("seed QA 失败: %w", err)
	}
	logJSON("seed.final", seed)
	...
	logJSON("bible.initial", bible)
	bible, _, err = qaAndReviseBible(ctx, architect, qaAgent, parser, req, seed, bible)
	if err != nil {
		return ScenarioCreationOutput{}, fmt.Errorf("bible QA 失败: %w", err)
	}
	logJSON("bible.final", bible)

	verifiedBible, err := verifyBibleRules(ctx, architect, parser, bible)
	if err != nil {
		return ScenarioCreationOutput{}, fmt.Errorf("规则核验失败: %w", err)
	}
	logJSON("verified_bible", verifiedBible)
	...
	logJSON("draft.initial", draft)

	const maxQAAttempts = 50
	...
	for attempt := 0; attempt < maxQAAttempts; attempt++ {
		...
		logJSON(fmt.Sprintf("draft.qa.attempt_%02d", attempt+1), qaResult)
		...
		draft = revised
		logJSON(fmt.Sprintf("draft.revision_%02d", attempt+1), draft)
	}
...
func runArtifactQA(ctx context.Context, qaAgent, parser agentHandle, stage string, req ScenarioCreationRequest, artifact any) (qaGuardResult, error) {
	reqJSON, _ := json.Marshal(req)
	artifactJSON, _ := json.Marshal(artifact)
	userMsg := fmt.Sprintf("请审查stage=%s的阶段产物。必须逐句审查artifact_json中所有文本字段；只输出QA JSON。\n\n<stage>\n%s\n</stage>\n\n<request_json>\n%s\n</request_json>\n\n<artifact_json>\n%s\n</artifact_json>",
		stage, stage, string(reqJSON), string(artifactJSON))
	msgs := []llm.ChatMessage{
		{Role: "system", Content: qaAgent.systemPrompt(artifactQASystemPrompt)},
		{Role: "user", Content: userMsg},
	}
	...
	if err != nil {
		return qaGuardResult{}, fmt.Errorf("%s artifact QA JSON 解析失败: %w", stage, err)
	}
	return result, nil
}

func qaAndReviseSeed(ctx context.Context, architect, qaAgent, parser agentHandle, req ScenarioCreationRequest, geographyChain []string, seed ScenarioSeed) (ScenarioSeed, qaGuardResult, error) {
	const maxArtifactQAAttempts = 50
	var qaResult qaGuardResult
	for attempt := 0; attempt < maxArtifactQAAttempts; attempt++ {
		if ctx.Err() != nil {
			return seed, qaGuardResult{}, ctx.Err()
		}
		var err error
		qaResult, err = runArtifactQA(ctx, qaAgent, parser, "seed", req, seed)
		if err != nil {
			return seed, qaGuardResult{}, err
		}
		logJSON(fmt.Sprintf("seed.qa.attempt_%02d", attempt+1), qaResult)
		if qaResult.Pass {
			return seed, qaResult, nil
		}
		if len(qaResult.MustFix) == 0 {
			return seed, qaResult, fmt.Errorf("seed QA 未通过但没有返回 must_fix")
		}
		if attempt == maxArtifactQAAttempts-1 {
			break
		}
		revised, err := reviseSeed(ctx, architect, parser, req, geographyChain, seed, qaResult.MustFix)
		if err != nil {
			return seed, qaResult, fmt.Errorf("seed 修订失败: %w", err)
		}
		seed = revised
		logJSON(fmt.Sprintf("seed.revision_%02d", attempt+1), seed)
	}
	return seed, qaResult, fmt.Errorf("seed QA 经过%d次仍未通过", maxArtifactQAAttempts)
}

func reviseSeed(ctx context.Context, architect, parser agentHandle, req ScenarioCreationRequest, geographyChain []string, seed ScenarioSeed, mustFix []string) (ScenarioSeed, error) {
	reqJSON, _ := json.Marshal(req)
	seedJSON, _ := json.Marshal(seed)
	userMsg := fmt.Sprintf(seedRevisionPrompt, string(reqJSON), strings.Join(geographyChain, " → "), string(seedJSON), formatMustFix(mustFix), scenarioSeedExample)
	msgs := []llm.ChatMessage{
		{Role: "system", Content: "你是 COC7 ScenarioSeed 定向修订器。只修QA指出的硬逻辑问题。仅输出修订后的完整 JSON,不要有其他文字。"},
		{Role: "user", Content: userMsg},
	}

	var revised ScenarioSeed
	if err := chatAndParseJSON(ctx, architect, parser, msgs, &revised, scenarioSeedExample, "seed.revision"); err != nil {
		return ScenarioSeed{}, err
	}
	return revised, nil
}

func qaAndReviseBible(ctx context.Context, architect, qaAgent, parser agentHandle, req ScenarioCreationRequest, seed ScenarioSeed, bible ScenarioBible) (ScenarioBible, qaGuardResult, error) {
	const maxArtifactQAAttempts = 50
	var qaResult qaGuardResult
	for attempt := 0; attempt < maxArtifactQAAttempts; attempt++ {
		if ctx.Err() != nil {
			return bible, qaGuardResult{}, ctx.Err()
		}
		var err error
		qaArtifact := struct {
			Seed  ScenarioSeed  `json:"scenario_seed"`
			Bible ScenarioBible `json:"scenario_bible"`
		}{Seed: seed, Bible: bible}
		qaResult, err = runArtifactQA(ctx, qaAgent, parser, "bible", req, qaArtifact)
		if err != nil {
			return bible, qaGuardResult{}, err
		}
		logJSON(fmt.Sprintf("bible.qa.attempt_%02d", attempt+1), qaResult)
		if qaResult.Pass {
			return bible, qaResult, nil
		}
		if len(qaResult.MustFix) == 0 {
			return bible, qaResult, fmt.Errorf("bible QA 未通过但没有返回 must_fix")
		}
		if attempt == maxArtifactQAAttempts-1 {
			break
		}
		revised, err := reviseBible(ctx, architect, parser, req, seed, bible, qaResult.MustFix)
		if err != nil {
			return bible, qaResult, fmt.Errorf("bible 修订失败: %w", err)
		}
		bible = revised
		logJSON(fmt.Sprintf("bible.revision_%02d", attempt+1), bible)
	}
	return bible, qaResult, fmt.Errorf("bible QA 经过%d次仍未通过", maxArtifactQAAttempts)
}

func reviseBible(ctx context.Context, architect, parser agentHandle, req ScenarioCreationRequest, seed ScenarioSeed, bible ScenarioBible, mustFix []string) (ScenarioBible, error) {
	reqJSON, _ := json.Marshal(req)
	seedJSON, _ := json.Marshal(seed)
	bibleJSON, _ := json.Marshal(bible)
	userMsg := fmt.Sprintf(bibleRevisionPrompt, string(reqJSON), string(seedJSON), string(bibleJSON), formatMustFix(mustFix), scenarioBibleExample)
	msgs := []llm.ChatMessage{
		{Role: "system", Content: "你是 COC7 ScenarioBible 定向修订器。只修QA指出的硬逻辑问题。仅输出修订后的完整 JSON,不要有其他文字。"},
		{Role: "user", Content: userMsg},
	}

	var revised ScenarioBible
	if err := chatAndParseJSON(ctx, architect, parser, msgs, &revised, scenarioBibleExample, "bible.revision"); err != nil {
...
func logJSON(tag string, v any) {
	...
	if err != nil {
		log.Printf("[scripter] %s marshal failed: %v; value=%#v", tag, err, v)
		return
	}
	log.Printf("[scripter] %s:\n%s", tag, string(data))
}

func formatMustFix(mustFix []string) string {
	items := make([]string, 0, len(mustFix))
