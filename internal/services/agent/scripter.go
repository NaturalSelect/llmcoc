...
var topicThreatOrigins = []string{
	...
	"跨维度威胁（来自平行世界或其他维度的入侵）",
	"内在恐怖（调查员自身潜藏的黑暗面被触发）",
	"社会腐败（权力滥用、道德沦丧引发的危机）",
	"文化冲突（不同文化/信仰体系之间的碰撞引发的超自然事件）",
	"历史遗留问题（未解的古老事件或未完成的仪式引发的后续危机）",
	"自然灾害（被神话力量放大的地震、风暴等自然事件）",
	"精神污染（被神话存在影响的人群或区域，导致暴力犯罪或社会秩序崩溃）",
...
var topicTwists = []string{
	...
	"最不起眼的日常物品，是触发全部真相的唯一媒介",
	"任务简报本身是一个陷阱，一开始就被引向错误的目标",
	"真相并不唯一，多个合理的解释同时成立，选择哪一个将重塑世界",
	"故事发生的世界本身是一个梦境",
}
