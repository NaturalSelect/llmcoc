...
<task>严格按design.md“单页模组敏捷开发”五步输出ScenarioSeed：1选择冲突舞台，2撰写核心故事线，3划定同心圆沙盒，4连接线索，5填充NPC和证据。只做种子，不写完整剧本。</task>
...
<rules>
- situation.crime_scene 对应design.md第一步“选择冲突舞台”：只锁定一个最适合展开调查的核心现场/局面；不要把完整剧情、调查员入场、结局、神话解释塞进去。
- plot 必须逐项回答design.md第二步四个问题：背景调查、反派诞生/目标、调查员不来会怎样、玩家如何卷入。
- sandbox 必须对应design.md第三步同心圆：core=故事的锁/必须发生的核心场景；middle=非强制但提供额外信息或挑战；outer=背景幕。
- sandbox.core 必须是3-5个核心场景节点。每个节点都要写清event、why_required、state_change；禁止只写地点名、氛围名或可选支线。
- clue_connections 必须对应design.md第四步：每个关键情报至少两条获取路径，路径可来自core/middle/outer，但必须最终帮助推进核心区域。
- wrap_up 必须对应design.md第五步：NPC要有鲜明人格和功能；evidence_props要是玩家可见、可引用的证据或handout种子。
...
func RunScripterScenarioTeam(ctx context.Context, req ScenarioCreationRequest) (ScenarioCreationOutput, error) {
	...
	log.Printf("[scripter] seed crime_scene=%q core_scenes=%d", seed.Situation.CrimeScene, len(seed.Sandbox.Core))
